        ...
        function exec() {
            return apkAction.__awaiter(this, void 0, void 0, function* () {
                ...
                try {
                    ...
                    //  拷贝sdk的所有资源(libs、res、code...)
                    if( sdk_path ) {
                        yield me.packingAction.copyResource(sdk_path, decomplilePath, package_name);
                    }
                    //  合并Manifest
                    if( sdk_path ) {
                        me.packingAction.mergeManifest(sdk_path, decomplilePath, package_name);
                    }
                    ...
                    try {
                        ...
                        var str = me.packingAction.read(cfgPath);
                    ...
                    if (genCache) {
                        if (!fs.existsSync(res_path)) {
                            if (url.length > 0) {
                                if (confirm(`指定的资源目录不存在！如果继续，包中将不包含资源
                                        ${res_path}还要继续打包么？`)) {
                                    genCache = false;
                                }
                                else {
