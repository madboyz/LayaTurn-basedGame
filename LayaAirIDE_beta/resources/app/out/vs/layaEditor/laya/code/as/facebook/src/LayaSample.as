package {
	import laya.events.Event;
	import laya.facebook.ConnectedPlayer;
	import laya.facebook.FBInstant;
	import laya.facebook.SharePayload;
	import laya.net.Loader;
	import laya.utils.Browser;
	import laya.utils.Handler;
	
	import ui.FBGameUI;

	public class LayaSample 
	{
		private var myPage:FBGameUI;
		private var FB:FBInstant;
		private var playerId:String;
		public function LayaSample() {
			//初始化引擎
			Laya.init(720, 1280);
			Laya.stage.scaleMode = "fixedwidth";
			//加载界面资源
			Laya.loader.load([{url:"res/atlas/comp.atlas",type:Loader.ATLAS}],new Handler(this,completed));
			
		}				
		private function completed():void
		{
			//实例界面
			myPage = new FBGameUI();
			myPage.btnShare.on(Event.CLICK,this,onBtnShare);
			myPage.btnLogout.on(Event.CLICK,this,onBtnLogout);
			myPage.btnSave.on(Event.CLICK,this,onBtnSave);
			myPage.btnLoading.on(Event.CLICK,this,onBtnLoading);
			myPage.btnFriend.on(Event.CLICK,this,onBtnFriend)
			myPage.btnChoose.on(Event.CLICK,this,onBtnChoose)
			//FB初始化完成
			FB = Browser.window.FBSDK;
			FB.initializeAsync(initializeAsync.bind(this));
			Laya.stage.addChild(myPage);
		}
		private function initializeAsync():void
		{
			myPage.locale.text =  "Locale:"+FB.getLocale();    //用户的地域信息，例如:zh_CN en_US
			myPage.id.text = "ID:"+FB.player.getID();
			myPage.name.text = "Name:"+FB.player.getName();
			myPage.photo.skin =  FB.player.getPhoto();//人物头像
			myPage.platform.text = "Platform:"+FB.getPlatform();  //运行的平台信息: IOS | ANDROID | WEB | MOBILE_WEB
			myPage.type.text = "Type:"+FB.context.getType();  //  游戏的来源类型："POST" | "THREAD" | "GROUP" | "SOLO"
			myPage.vision.text = "Vision"+FB.getSDKVersion();  //SDK 的版本号
			//获取当前SDK支持的api
			trace("API:" +FB.getSupportedAPIs());				
			FB.player.getSignedPlayerInfoAsync(function(data):void{
				trace(data.$SignedPlayerInfo1.playerID+",,,,signature:"+data.$SignedPlayerInfo1.signature);
			});
			//设置facebook下游戏加载进度
			FB.setLoadingProgress(30);
			FB.startGameAsync(function(data):void{
				console.log("加载界面已经移除可以开始游戏"+data);
				//获取游戏Id,需要在startGameAsync之后才能获取到，之前有可能没有
				myPage.gameId.text = "gameId:"+FB.context.getID();
			});
		}
		/**
		 * 选择游戏场景 
		 * 
		 */		
		private function onBtnChoose():void
		{			
			alert("调用了'选择游戏场景'接口,此项目为模拟项目,具体效果请在facebook环境下调试");
			FB.context.chooseAsync(playerId,function():void{
			
			});
		}
		/**
		 * 获取同玩好友 
		 * 
		 */		
		private function onBtnFriend():void
		{			
			alert("调用了'同玩好友'接口,此项目为模拟项目,具体效果请在facebook环境下调试");
			//获取同玩的好友，关系链
			FB.player.getConnectedPlayersAsync(function(data):void{
				for(var i:int = 0;i<data.length;i++){
					var a:ConnectedPlayer = data[i];
					console.log("getConnectedPlayersAsync:"+"ID:"+a.getID()+"Name:"+a.getName()+"photo:"+a.getPhoto());//[ConnectedPlayer]
				}
				//获取下某个好友的ID
				playerId = data[0].getID();
			})
		}
		/**
		 * 获取远程数据 
		 * 
		 */		
		private function onBtnLoading():void
		{			
			alert("调用了'获取远程数据'接口,此项目为模拟项目,具体效果请在facebook环境下调试");
			FB.player.getDataAsync(["name","data"],function (data):void {
				alert("getDataAsync"+JSON.stringify(data));
			});
		}
		/**
		 * 保存上传数据 
		 * 
		 */		
		private function onBtnSave():void
		{			
			alert("调用了'保存数据'接口,此项目为模拟项目,具体效果请在facebook环境下调试");
			//上传自定义数据到平台
			FB.player.setDataAsync({name:"setDataAsync",data:"2333333"},function ():void {
				//从平台获取自定义数据
				alert("setDataAsync");
				//立即更新数据到平台，谨慎使用，
				FB.player.flushDataAsync(function ():void {
					alert("flushDataAsync");
				});
			});
		}
		/**
		 * 退出游戏 
		 * 
		 */		
		private function onBtnLogout():void
		{
			alert("调用了'退出游戏'接口,此项目为模拟项目,具体效果请在facebook环境下调试");
			//退出游戏
			FB.quit();
		}
		/**
		 * 分享 
		 * 
		 */		
		private function onBtnShare():void
		{
			alert("调用了'分享 '接口,此项目为模拟项目,具体效果请在facebook环境下调试");
			var payload:SharePayload = new Browser.window.SharePayload();
			payload.intent = "SHARE";
			//需要base64编码
			payload.image = "data:image/png;base64,";
			payload.text = "分享";
			payload.data = {name:"1",id:"2"};
			FB.shareAsync(payload,function():void{
				alert("分享");
			});
		}
	}
}