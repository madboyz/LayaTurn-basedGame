package view {
	...
	public class TestView extends TestPageUI {
		...
		public function TestView() {
			...
			btnShare.on(Event.CLICK,this,onBtnShare);
			btnLogout.on(Event.CLICK,this,onBtnLogout);
			btnSave.on(Event.CLICK,this,onBtnSave);
			btnLoading.on(Event.CLICK,this,onBtnLoading);
			btnFriend.on(Event.CLICK,this,onBtnFriend)
			btnChoose.on(Event.CLICK,this,onBtnChoose)
		}
		...
		private function initializeAsync():void
		{
			...
			Locale.text =  "Locale:"+FB.getLocale();    //用户的地域信息，例如:zh_CN en_US
			ID.text = "ID:"+FB.player.getID();
			Name.text = "Name:"+FB.player.getName();
			photo.skin =  FB.player.getPhoto();//人物头像
			Platform.text = "Platform:"+FB.getPlatform();  //运行的平台信息: IOS | ANDROID | WEB | MOBILE_WEB
			Type.text = "Type:"+FB.context.getType();  //  游戏的来源类型："POST" | "THREAD" | "GROUP" | "SOLO"
			Vision.text = "Vision"+FB.getSDKVersion();  //SDK 的版本号
			//获取当前SDK支持的api
			...
			FB.startGameAsync(function(data):void{
				...
				gameId.text = "gameId:"+FB.context.getID();
			});
		}
	}
}