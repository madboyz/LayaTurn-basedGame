...
class CodeMain extends EventEmitter {
    constructor(args) {
        ...
        this.layaIDEVersion = "1.8.1beta";
        this.localversion = "1.9.1.1600";
        this.portrange = 8900;
    ...
    initAdvInfo() {
        let infoObj = {
            url: `https://ldc.layabox.com/download/ideinfo/adconfig.json?v=${Date.now()}`
        };
        this.getInfoFromServer(infoObj, (re) => {
            ...
            if (advObj.topbarad) {
                ...
                menuAdvertisingEle.parentElement.style = topbarObj.parentStyle1;
            }
            if (advObj.webviewad) {
                ...
                for (let i = 0; i < length; i++) {
                    ...
                    if (isVip && webviewItem.viphide) { // 是VIP会员，并且该广告不对VIP展示，关闭
                        continue;
                    } else if (this.isAdvOpenToday(id)) { // 每天只出现一次
                        continue;
                    }
                    this.saveIdeadvInfo(id);
                    url = webviewItem.url;
     ...
     * 获取记录的广告打开信息
     * - 如果传入id，则记录该id打开的时间
     * - 如果传入id，则可能返回空值，否则至少返回空对象
     */
    getIdeAdvInfo(advId) {
        let ideadvStr = localStorage.getItem("ideadvstorage");
        let ideadvInfo = {};
        try {
            ideadvInfo = ideadvStr && JSON.parse(ideadvStr);
        } catch(e) {}
        if (!ideadvInfo || "object" !== typeof ideadvInfo) {
            ideadvInfo = {};
        }
        if (advId) {
            return ideadvInfo[advId];
        }
        return ideadvInfo;
    }
     ...
     * - false 今天没打开过
     */
    isAdvOpenToday(advId) {
        if (!advId) {
            return false;
        }
        let advtime = this.getIdeAdvInfo(advId);
        if (advtime) {
            let advShowTime = new Date(advtime.time);
            let nowTime = new Date();
            // 时间在一天内，并且属于物理同一天
            if (nowTime - advShowTime < 86400000 && advShowTime.getDate() === nowTime.getDate()) {
                return true;
            }
        }
        return false;
     ...
     * 传入广告id，记录广告打开时间
     */
    saveIdeadvInfo(advId) {
        if (!advId) {
            return;
        }
        let ideadvInfo = this.getIdeAdvInfo();
        ideadvInfo[advId] = {
            time: Date.now()
        }
        localStorage.setItem("ideadvstorage", JSON.stringify(ideadvInfo));
    }
     ...
     */
    addMenuListItemInCode(id, itemName, menuSocketName) {
        let idList = ["file", "edit", "selection", "view", "go", "tool", "prolaya", "help"];
        let _id = idList.indexOf(id) !== -1 ? id : "help";
        codeMenuDiv.addNewChild(_id, 5, menuSocketName, itemName);
    }
    onbeforeHandler() {
        var t = 1;
    ...
    initRecentOpenList() {
        ...
        function readProjectInfoASync(fileList, index) {
            ...
            // 是否需要特殊处理 mac/linux 平台?
            let filePath = codeMain.workspacePath.split('\\');
            filePath.push(fileList[index]);
            let p = codeMain.path.join.apply(codeMain.path, filePath);
        ...
        // 读取配置信息
        let p = this.path.join.apply(this.path, this.workspacePath.split('\\'));
    ...
    publishPro2() {
        ...
        let envPATHStr = process.platform === 'darwin' ? `&envPATH=${process.env.PATH}` : null;
    ...
    menuclickHandler(comamand) {
        ...
        switch (comamand) {
            ...
            case "checkupdateVersions":
                ...
                console.log(isBeta, oldVersion, newVersion);
                // 没有更新
    ...
    setUpCodeMenu() {
        setTimeout(() => {
            let proInfo = window.proInfoObj;
            // 10、11为HPB区块链项目，因为2.0将10分配给了MakerGame，1.0发版时错误的将10分配给了HPB项目，无法挽回，所以将10、11都分配给HPB
            if (proInfo && (proInfo.layaProType === 7 || proInfo.layaProType === 9 || proInfo.layaProType === 10 || proInfo.layaProType === 11)) { // 如果是区块链项目
