...
function NewProLaya()
{
    ...
    function mouseDownHandler(e)
    {
        ...
        img.src = codeMain.newProInfoList[index].imgSrc;
    ...
    skipBtn.onmousedown = function(e)
        {
            ...
            // 路径中不能有中文或其他特殊符号
            if (!proFilePath.value.match(/^[\w\s:/\\\.]+$/)) {
            ...
            if (isNeedCopyLib && proSelectType.selectedIndex == 0)
            {
            ...
            else if (isNeedCopyLib && proSelectType.selectedIndex == 1)
            {
            ...
            if (!isNeedCopyLib) {
                ...
                // 新建目录
                codeMain.mkdirsSync(proFilePath.value);
            }
...
function blockchainDeploy(params, cb) {
    ...
    isHPBProject =  window.proInfoObj && (window.proInfoObj.layaProType === 10 || window.proInfoObj.layaProType === 11);
    NEOCSFilePath = null;
    if (!isETHProject && !isNEOProject && !isHPBProject) { // 因为菜单是动态注入的，一般而言不会执行该代码
        alert(codeMain.getPanelLang(306)); // 请在区块链项目中使用部署功能!
        return;
    }
    ...
    <div class="layerBackground">
        ...
        <li class="closeBtn"><span style="font-size: 24px;">×</span></li>
    </div>
    `;
    // 动态改变右侧核心内容
    ...
    closeBtn = blockchainEle.getElementsByClassName("closeBtn")[0];
...
function picToolsLaya()
{
    ...
    inputFile.placeholder = `${codeMain.panel[487][langindex]}`//"资源文件夹的父级目录...";
