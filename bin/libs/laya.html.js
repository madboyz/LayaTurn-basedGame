	...
	*/
	...
	__getset(0,__proto,'src',null,function(url){
		...
		function onloaded (){
			 ...
			 * 2019/02/20 fixBywangwx Loader.cacheRes应该再彻底load完后再cache否则其他模块需要加载同一个资源就会为null
			 */
			if (Loader.loadedMap[URL.formatURL(url)]==null){
				Loader.cacheRes(_$this._url,_$this._tex);
			}
			_$this.repaint();
