...
export class PlayerTitleItem extends ui.main.PlayerTitleItemUI {
    private info:any;
    constructor() {
        super();
    }
    ...
    {
        if(!this.info)return;
        this.nameTxt.text = this.info.sheet.female_name;
        var isHave = this.info.isHave;
    }
    ...
    public checkSelect(vo:any)
    {
        ...
        if(SRoleData.instance.info.GraphTitle == 0)
        this.IsInstall.visible = false;
        else if(SRoleData.instance.info.GraphTitle == this.info.sheet.no)
