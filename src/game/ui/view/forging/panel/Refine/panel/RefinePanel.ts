...
import { RefineEquipPart } from "./RefineEquipPart";
...
export class RefinePanel extends StrengthPanel {
    constructor() {
        super();
    }
    ...
    {
        this.btn_str.label = "一键精炼";
    }

    public initPart():void
    {
        this.equipPart = new RefineEquipPart();
    ...
    {
        this.curInfo = SForgingData.instance.curRefineInfo;
        ...
        if(this.curEquipItem && this.curInfo)
        {
            ...
            if(baseArr)
            {
                for (let index = 0; index < baseArr.length; index++) {
                    ...
                    var curNum = (this.curInfo as RefineInfo).addAttrNum(base.atrrName,baseNum);
                    var nextNum = (this.curInfo as RefineInfo).addAttrNum(base.atrrName,baseNum,true);
        ...
        if(!this.curInfo.max)
        {
            if(this.curInfo.needEquipLv <= SRoleData.instance.info.Lv)
            {
                if(this.curInfo.canStrength)
                {
                    ...
                    this.parent.event(SForgingEvent.FORGING_REQUEST_STRENGTH,[[StrengthType.REFINE,this.curInfo.Slot,1,0]]);
                }
                else
                {
                    this.isStarAuto = false;
