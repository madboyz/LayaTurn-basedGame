...
import { S57033_2 } from "../../../../../../net/pt/pt_57";
...
export class ChuangTiangongPanel extends ui.main.SanjieCopyPanelUI {
    private stage1: ChuangTiangongItem;
    private stage2: ChuangTiangongItem;
    private stage3: ChuangTiangongItem;
    ...
    private initList(): void {
        if (!this.stage1) {
            this.stage1 = new ChuangTiangongItem;
            this.stage1.x = 72;
            this.stage1.y = 264;
            this.addChild(this.stage1);
        }
        if (!this.stage2) {
            this.stage2 = new ChuangTiangongItem;
            this.stage2.x = 225;
            this.stage2.y = 264;
            this.addChild(this.stage2);
        }
        if (!this.stage3) {
            this.stage3 = new ChuangTiangongItem;
            this.stage3.x = 378;
            this.stage3.y = 264;
    ...
    private requestInfo(): void {
        (new RankMainProrocol()).RequsetRank(RankType.CHUANGTIANGONG, 4, 1);
    }

    private initEvent(): void {
        DataManager.listen(PROTOCOL.E22001, this, this.onS22001);//个人排行榜
        SBossData.instance.on(SBossEvent.CHUANGTIANGONG_INFO, this, this.updateData);
    ...
    private removeEvent(): void {
        ...
        SBossData.instance.off(SBossEvent.CHUANGTIANGONG_INFO, this, this.updateData);
    ...
    public onOpen(): void {
        this.cfgs = FbVo.getListByType(CopyType.CHUANGTIANGONG);
        this.boxCfgs = [];
        ...
        this.nowDunNo = SBossData.instance.chuangTiangongChapInfo.DunNo - 25001;
        ...
        var useCfg: FbVo;
        for (let i = 0; i < this.cfgs.length; i++) {
            if (this.cfgs[i].no >= SBossData.instance.chuangTiangongChapInfo.DunNo && this.cfgs[i].resource && this.cfgs[i].resource.length > 0) {
                useCfg = this.cfgs[i];
                break;
            }
        }
        ...
        //初始化选择
        this.selectDunNo = SBossData.instance.getCTGMaxCanFightDunNo();
        this.selectDunNoPage = this.getCTGMaxPageIndex();
        this.selectBoxPage = this.getCTGMaxBoxPageIndex();
    ...
    public updateData(): void {
        ...
        var maxStage = this.getCTGMaxNoPassPageIndex() + 1;
        this.stageBtnR.gray = this.selectDunNoPage >= maxStage;
        this.boxBtnL.gray = this.selectBoxPage <= 0;
        var maxBoxPage = this.getCTGMaxBoxcanSeePage();
        this.boxBtnR.gray = this.selectBoxPage >= maxBoxPage;

        //进度条
        this.proBar.setValue(5, 100);
        //打相关按钮
        var info: S57033_2 = SBossData.instance.getChuangTiangongInfo(this.selectDunNo);
        ...
        for (let i = 0; i <= 3; i++) {
            ...
            var starCount = startCfg != null ? (startCfg.no - 25000) : 0;
            var endCfg = this.boxCfgs[this.selectBoxPage * 3 + i];
            var endCount = endCfg != null ? (endCfg.no - 25000) : 10000;
            var nowCount = SBossData.instance.chuangTiangongChapInfo.DunNo - 25001;
        ...
        for (let i = 1; i <= 3; i++) {
            ...
            if (cfg) {
                var info = SBossData.instance.getChuangTiangongInfo(cfg.no);
                var canGet = (SBossData.instance.chuangTiangongChapInfo.DunNo - 1) >= cfg.no;
                var hadGet = info && info.RwdState == 3;
                boxStage1.text = (cfg.no - 25000) + "层";
                if (!canGet) {
    ...
    private getCTGMaxPageIndex(): number {
        var fightIndex = SBossData.instance.getCTGMaxCanFightIndex();
        return Math.floor(fightIndex / 3);
    }

    private getCTGMaxNoPassPageIndex(): number {
        var noPassIndex = SBossData.instance.getCTGMaxNoPassIndex();
        return Math.floor(noPassIndex / 3);
    }

    private getCTGMaxBoxPageIndex(): number {
        var maxBoxIndex: number = 0;
        for (let i = 0; i < this.boxCfgs.length; i++) {
            var ele = this.boxCfgs[i];
            var info = SBossData.instance.getChuangTiangongInfo(ele.no);
    ...
    private getCTGMaxBoxcanSeePage(): number {
        var maxBoxIndex: number = 0;
        for (let i = 0; i < this.boxCfgs.length; i++) {
            var ele = this.boxCfgs[i];
            if (ele.no >= SBossData.instance.chuangTiangongChapInfo.DunNo) {
                maxBoxIndex = i;
                break;
            }
        }
    ...
    private onS22001(data: S22001) {
        if (data.RankType == RankType.CHUANGTIANGONG) {
    ...
    public rankTxtClick(): void {
        UIManager.instance.openUI(UIID.CHUANGTIANGONG_RANK_PANEL);
    }
    ...
    public boxClick(index: number): void {
        ...
        UIManager.instance.openUI(UIID.COM_SHOW_REWARD, [
            ...
            null,//点击确定的方法
            "通关奖励",//一级标题
            "第" + (useCfg.no - 25000) + "层通关奖励",//二级标题
            "确定",//确定按钮名字
        ]);

        var info = SBossData.instance.getChuangTiangongInfo(useCfg.no);
        var canGet = (SBossData.instance.chuangTiangongChapInfo.DunNo - 1) >= useCfg.no;
        var hadGet = info && info.RwdState == 3;
        ...
        if (!canGet) {
            ...
            SBossData.instance.protocol.send57035(useCfg.no);
        }
    ...
    private btn_enterClick(): void {
        if (this.selectDunNo > SBossData.instance.chuangTiangongChapInfo.DunNo) {
            MsgManager.instance.showRollTipsMsg("请先通关前置关卡");
            return;
        }
        SCopyData.instance.copyType = CopyType.CHUANGTIANGONG;
    ...
    private btn_sweepClick(): void {
        ...
        var haveYueka = SRechargeData.instance.haveYueka();
        if (haveYueka) {
            SCopyData.instance.oneKeyShaodang(CopyType.CHUANGTIANGONG);
        } else {
            SBossData.instance.protocol.send57034(this.selectDunNo);
        }
    }
...
//道具的奖励ITEM
export class ChuangTiangongItem extends ui.main.SanjieCopyItemUI {
    private _mData: FbVo;
    ...
    public set dataSource(data: FbVo) {
        ...
        //首通
        this.titleLb.text = `第${(data.no - 25000)}层`;

        var panel = (this.parent as ChuangTiangongPanel)
        ...
        var chapInfo = SBossData.instance.getChuangTiangongInfo(data.no);
