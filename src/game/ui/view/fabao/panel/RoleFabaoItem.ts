...
export class RoleFabaoItem extends BaseItem{
    public _type:number;
    ...
    {
        if(!this.itemData && SBagData.instance.fabao.hasBatterEquipByType(this.type))
        {
            if(!this._red)
            {
                this._red = new Laya.Image();
                this._red.skin = ResUtils.getCompUIUrl("img_red");
                this._red.x = this.width - this._red.width + 8;
                this._red.y = -8;
