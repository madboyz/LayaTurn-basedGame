...
export class ComateEquipPart extends Laya.Sprite {
    ...
    public updateAllEquip(selectVo: ComateInfo = null): void {
        ...
        for (let i = 1; i < this._equipTypeList.length; i++) {
            ...
            var equip: S15000_1 = SBagData.instance.comateEquip.getPetEquip(this.selectVo, type);
            var hasBetterEquip = SBagData.instance.comateEquip.checkOneCanEquip(this.selectVo, type);
        ...
        for (let j = 1; j < this._equipTypeList.length; j++) {
            ...
            var showBtn = SBagData.instance.comateEquip.checkOneCanEquip(this.selectVo, type);
            if (showBtn) {
    ...
    public btnClick(): void {
        for (let j = 1; j < this._equipTypeList.length; j++) {
            var type = this._equipTypeList[j].type;
            var betterItem: ItemData = SBagData.instance.comateEquip.findOneCanEquip(this.selectVo, type);
            if (betterItem) {
                CommonControl.instance.onEquipInstall(betterItem.GoodsId, this.selectVo.Id, 2/**2为伙伴 */);
            }
        }
    }
